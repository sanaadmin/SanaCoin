...
{
    switch(vin_val) {
        case 500000 * COIN: return 1;
        case 2500000 * COIN: return 2;
...
*/
    ...
    // should be at least not earlier than block when 1000 SanaCoin tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && mi->second) {
        CBlockIndex* pMNIndex = mi->second;                                                        // block for 1000 SanaCoin tx -> 1 confirmation
