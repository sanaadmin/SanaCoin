...
#if defined(NDEBUG)
#error "SanaCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usanacoin) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {    // premine
      nSubsidy = 2000000000 * COIN;         // <- 프리마이닝 갯수
    } else if (nHeight <= 2000) {           
      nSubsidy = 0.1 * COIN;
    } else {
      nSubsidy = 100 * COIN;                // pos 마이닝 갯수
    }
...
{
    RenameThread("sanacoin-scriptch");
